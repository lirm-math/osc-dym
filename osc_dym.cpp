...
int main(int argc, char* args[])
{
    ...
    vector<real_t> x = {
        ...
        x0 + dx,
        x0 + 2 * dx,
        // x0 + 3 * dx,  // you may have more points on the x-direction
        // x0 + 4 * dx,
        // x0 + 5 * dx,
    };
    ...
    vector<real_t> t = {
        ...
        t0 + dt,
        t0 + 2 * dt,
        // t0 + 3 * dt,  // and on the t-direction, too
        // t0 + 4 * dt,
        // t0 + 5 * dt,
    };
    ...
    const vector<real_t> z = { "0.3"_r, "0.3"_r};

    /// Phase parameters
    const vector<real_t> omega = { "0"_r, "0"_r};
    ...
    compute_derivatives(dx, dt, x.size(), t.size(),
        S, S00, S10, S20, S01);
    compute_derivatives(dx, dt, x.size(), t.size(),
        X, X00, X10, X20, X01);
    compute_derivatives(dx, dt, x.size(), t.size(),
    ...
    return (epsilon < "1e-6"_r) ? 0 : 1;  // Return success if error is below tolerance
}